...
import { StdErrError } from '../errors/stdErr';
...
export class ProcessService implements IProcessService {
    constructor(private readonly decoder: IBufferDecoder, private readonly env?: NodeJS.ProcessEnv) { }
    public execObservable(file: string, args: string[], options: SpawnOptions = {}): ObservableExecutionResult {
        ...
        const output = new Observable<Output>(subscriber => {
    ...
    public exec(file: string, args: string[], options: SpawnOptions = {}): Promise<ExecutionResult> {
        const encoding = options.encoding = typeof options.encoding === 'string' && options.encoding.length > 0 ? options.encoding : DEFAULT_ENCODING;
        delete options.encoding;
        const spawnOptions = { ...options };
        if (!spawnOptions.env || Object.keys(spawnOptions).length === 0) {
